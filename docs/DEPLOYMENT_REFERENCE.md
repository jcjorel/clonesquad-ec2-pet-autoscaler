...
These data are generated on-demand (through the [API Gateway](INTERACTING.md#api-backup")) or periodically with a [cron settings](CONFIGURATION_REFERENCE.md#cronbackup).

> Note: **Setting this parameter automatically activates an hourly backup and metadata generation**. Please [see documentation](BACKUP_AND_METADATA.md) if this behavior needs to be modified.
