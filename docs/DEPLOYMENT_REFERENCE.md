...
The 2 optional parameters defined below are used to populate the 'ApiGwVpcEndpointDNSEntry' field in the [Discovery API](INTERACTING.md#api-discovery)/Lambda. If not set, the VPC endpoint DNS name is guessed from [`ApiGWEndpointConfiguration`](#apigwendpointconfiguration) (if defined). Setting these parameters is useful to propagate the VPC Endpoint DNS names to external software that needs an automatic discovery (The [cs-instance-watcher](TOOLS.md#cs-instance-watcher) is using this information). **This parameter is mainly useful when the endpoint is configured with `PrivateDnsEnabled` set to `False`.**

* VpcEndpointDNS: (Optional) When [`ApiGWEndpointConfiguration`](#apigwendpointconfiguration) is not defined, this value is used. 
